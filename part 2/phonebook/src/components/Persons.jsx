function Persons({ personsToShow, handleDeleteUser }) {
    return (
        <section className='phonebook__numbers phb-numbers'>
            ...
            {personsToShow.map(item => <li key={item.id} className='phb-list__item'><span>{item.name}</span> <span>{item.number}</span> <button onClick={() => handleDeleteUser(item)}>Delete</button></li>)}
            </ul>
        </section> 
    );
}
